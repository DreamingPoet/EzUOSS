    ...
    if (index == -1) {
        if (use_api_index == API_STACK_SIZE * 3 / 4) {
            ...
            if (api_switch == NULL) {
                ...
                errno_t err = EOK;
                memcpy_s(api_switch, sizeof(obs_s3_switch) * API_STACK_SIZE, temp_api_switch, sizeof(obs_s3_switch) * API_STACK_SIZE / 2);
                if (err != EOK)
                {
...
void set_use_api_switch(const obs_options* options, obs_use_api* use_api_temp)
{
    ...
    if (use_api_index == -1) {
        ...
        {
            err = EOK; 
            memcpy_s(api_switch[use_api_index].bucket_name, BUCKET_LEN - 1, options->bucket_options.bucket_name, strlen(options->bucket_options.bucket_name));
            // CheckAndLogNoneZero(err, "memcpy_s", __FUNCTION__, __LINE__);

            api_switch[use_api_index].bucket_name[strlen(options->bucket_options.bucket_name)] = '\0';

            err = EOK; 
            memcpy_s(api_switch[use_api_index].host_name, DOMAIN_LEN - 1, options->bucket_options.host_name, strlen(options->bucket_options.host_name));
            // CheckAndLogNoneZero(err, "memcpy_s", __FUNCTION__, __LINE__);
