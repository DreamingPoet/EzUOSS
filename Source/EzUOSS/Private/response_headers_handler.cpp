...
void response_headers_handler::response_headers_handler_initialize(response_headers_handler *handler)
{
	FMemory::Memset(&handler->responseProperties, 0, sizeof(obs_response_properties));
	...
	int namelen, int valuelen, char *c)
{
	...
	if (prefix_cmp(header, "x-amz-request-id", namelen)||prefix_cmp(header, "x-obs_-request-id", namelen)) {
	...
	else if (prefix_cmp(header, "x-reserved-indicator", namelen)) {
		responseProperties->reserved_indicator = 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-amz-id-2", namelen)||prefix_cmp(header, "x-obs_-id-2", namelen)) {
		responseProperties->request_id2 = 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "Content-Type", namelen)) {
		responseProperties->content_type = 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "Content-Length", namelen)) {
	...
	else if (prefix_cmp(header, "Server", namelen)) {
		responseProperties->server = 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "ETag", namelen)) {
		responseProperties->etag = 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-amz-expiration", namelen)||prefix_cmp(header, "x-obs_-expiration", namelen)) {
		responseProperties->expiration= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if(prefix_cmp(header, "x-amz-website-redirect-location", namelen)||prefix_cmp(header, "x-obs_-website-redirect-location", namelen)) {
		responseProperties->website_redirect_location= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-amz-version-id", namelen)||prefix_cmp(header, "x-obs_-version-id", MAX(namelen, 16))) {
		responseProperties->version_id= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "Access-Control-Allow-Origin", namelen)) {
		responseProperties->allow_origin= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "Access-Control-Allow-Headers", namelen)) {
		responseProperties->allow_headers= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "Access-Control-Max-Age", namelen)) {
		responseProperties->max_age= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "Access-Control-Allow-Methods", namelen)) {
		responseProperties->allow_methods= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "Access-Control-Expose-Headers", namelen)) {
		responseProperties->expose_headers= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-default-storage-class", namelen)||prefix_cmp(header, "x-obs_-storage-class", namelen)) {
		responseProperties->storage_class=
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-amz-storage-class", namelen)) {
		responseProperties->storage_class=
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-amz-server-side-encryption", namelen)||prefix_cmp(header, "x-obs_-server-side-encryption", namelen)) {
	...
	else if (prefix_cmp(header, "x-amz-server-side-encryption-aws-kms-key-id", namelen)||prefix_cmp(header, "x-obs_-server-side-encryption-aws-kms-key-id", namelen)) {
		responseProperties->kms_key_id= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-amz-server-side-encryption-customer-algorithm", namelen)||prefix_cmp(header, "x-obs_-server-side-encryption-customer-algorithm", namelen)) {
	...
	else if (prefix_cmp(header, "x-amz-server-side-encryption-customer-key-MD5", namelen)||prefix_cmp(header, "x-obs_-server-side-encryption-customer-key-MD5", namelen)) {
		responseProperties->customer_key_md5= 
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, 
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-obs_-bucket-location", namelen)) {
		responseProperties->bucket_location=
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-obs_-version", namelen)) {
		responseProperties->obs_version =
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-amz-restore", namelen)||prefix_cmp(header, "x-obs_-restore", namelen)) {
		responseProperties->restore =
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-obs_-object-type", namelen)) {
		responseProperties->obs_object_type =
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-obs_-next-append-position", namelen)) {
		responseProperties->obs_next_append_position =
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-amz-epid", namelen)) {
		responseProperties->obs_head_epid  =
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, "x-obs_-epid", namelen)) {
		responseProperties->obs_head_epid  =
			string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c,
			valuelen, fit);
	}
	else if (prefix_cmp(header, OBS_METADATA_HEADER_NAME_PREFIX, 
		sizeof(OBS_METADATA_HEADER_NAME_PREFIX) - 1) ||
		prefix_cmp(header, "x-obs_-meta-", sizeof("x-obs_-meta-") - 1)) 
	{
...
void response_headers_handler::response_headers_handler_add(response_headers_handler *handler,
                                  char *header, int len)
{
    char *end = &(header[len]);
    if (handler->done) {
...
void response_headers_handler::response_headers_handler_done(response_headers_handler *handler, CURL *curl)
{
